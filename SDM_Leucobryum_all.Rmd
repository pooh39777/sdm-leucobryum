---
title: "SDMs of Leucobryum (All Species)"
...
```{r Get bioclim data}
bio.files <- list.files("./wc2.1_2.5m_bio", ".tif$", full.names=T, recursive=T)
bioclim <- stack(bio.files)
names(bioclim) <- c("wc2_1","wc2_10","wc2_11","wc2_12","wc2_13","wc2_14","wc2_15","wc2_16","wc2_17","wc2_18","wc2_19","wc2_2","wc2_3","wc2_4","wc2_5","wc2_6","wc2_15","wc2_8","wc2_9")
```
...
```{r Import from .csv}
csv <- read_csv("data_20240406.csv")
```
...
occ.0 <- csv %>% 
  ...
  select(country, gbifID, genus, longitude, latitude) %>% 
  na.omit()
```
...
thin(occ.0, lat.col = "latitude", long.col = "longitude",
     spec.col = "genus", thin.par = 4.5, reps = 1,
     out.dir = getwd())
...
bg.point <- raptr::randomPoints(predictors, 10000)
```
...
data <- prepareSWD(species = "Leucobryum", p = occ[,2:3], a = bg.point, env = predictors)
...
swd2csv(data, file_name = "data_swd_Leucobryum.csv")
...
```{r Response curves}
plotResponse(tuned_model, var = "wc2_4", type = "cloglog", only_presence = TRUE, marginal = TRUE, fun = mean, rug = TRUE)
ggsave("response_curve_wc2_4.png", width = 7, height = 7, units = "cm")
ggsave("response_curve_wc2_4.pdf", width = 7, height = 7, units = "cm")

plotResponse(tuned_model, var = "wc2_15", type = "cloglog", only_presence = TRUE, marginal = TRUE, fun = mean, rug = TRUE)
ggsave("response_curve_wc2_15.png", width = 7, height = 7, units = "cm")
ggsave("response_curve_wc2_15.pdf", width = 7, height = 7, units = "cm")

plotResponse(tuned_model, var = "wc2_18", type = "cloglog", only_presence = TRUE, marginal = TRUE, fun = mean, rug = TRUE)
ggsave("response_curve_wc2_18.png", width = 7, height = 7, units = "cm")
ggsave("response_curve_wc2_18.pdf", width = 7, height = 7, units = "cm")

plotResponse(tuned_model, var = "wc2_8", type = "cloglog", only_presence = TRUE, marginal = TRUE, fun = mean, rug = TRUE)
ggsave("response_curve_wc2_8.png", width = 7, height = 7, units = "cm")
ggsave("response_curve_wc2_8.pdf", width = 7, height = 7, units = "cm")
```
...
ggplot() +
  ...
  scale_fill_manual(values = c("tomato", "grey", "cornflowerblue", "green"), 
                    ...
                    na.value = "NA") + 
  geom_jitter(data = gbif.csv, aes(x = lon, y = lat), color = "black", alpha = 0.4, size = 1) +
  scale_x_continuous(limits = c(95, 140)) +
