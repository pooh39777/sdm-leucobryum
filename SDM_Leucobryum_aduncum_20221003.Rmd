...
ggsave("map_current.png", width = 20, height = 15, units = "cm")
```
...
gplot(future.sum) +
    ...
    theme(plot.title = element_text(hjust = 0.5),
          axis.ticks.x = element_blank(),
          axis.ticks.y = element_blank())
ggsave("pa_map_future_sum.png", width = 20, height = 15, units = "cm")
...
future_freq <- as.data.frame(freq(future.sum, useNA = "no")) %>% 
  mutate(area_km2 = count/sum(count)*raster_area) %>% 
  mutate(percentage = round(count/sum(count)*100, 2))
future_freq

write.csv(future_freq, "future_freq.csv", row.names = FALSE)
```
...
ggplot() +
  ...
  scale_fill_manual(values = c("tomato", "grey", "cornflowerblue", "green"), 
                    name = "Class",
                    labels = c("Decrease", "Absent", "Consistent", "Increase", "Sea"),
                    na.value = "NA") + 
