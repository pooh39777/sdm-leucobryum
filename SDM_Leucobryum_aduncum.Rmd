---
title: "SDMs of Leucobryum Aduncum"
...
ext <- as(extent(95, 140, -10, 23), "SpatialPolygons")

#Fix bug, let R know the method
train(method = "Maxnet", data = data)
```
...
Get the available bioclim data from the worldclim dataset from [wc2.1_2.5m_bio.zip](https://biogeo.ucdavis.edu/data/worldclim/v2.1/base/wc2.1_2.5m_bio.zip)
See more at [Bioclimatic variables](https://www.worldclim.org/data/bioclim.html)
...
```{r Import from GBIF}
gbif <- gbif("leucobryum","aduncum") #1579 records found (2022-10-26)
```
...
```{r Plot species occurrence with mapview}
sp <- SpatialPoints(coords = cbind(gbif.csv$lon, gbif.csv$lat), proj4string = CRS("+init=epsg:4326"))
mapview(sp, cex = 3, col.regions = "red", col = "black")
```
...
```{r Response curves}
plotResponse(tuned_model, var = "wc2_9", type = "cloglog", only_presence = TRUE, marginal = TRUE, fun = mean, rug = TRUE)
ggsave("response_curve_wc2_9.png", width = 7, height = 7, units = "cm")
ggsave("response_curve_wc2_9.pdf", width = 7, height = 7, units = "cm")
