...
occ.0 <- csv %>% 
  filter(specificEpithet == "aduncum") %>% 
  #filter(fullCountry == "Thailand") %>%
  select(country, gbifID, genus, specificEpithet, longitude, latitude) %>% 
  na.omit()
```
...
data <- prepareSWD(species = "Leucobryum aduncum", p = occ[,2:3], a = bg.point, env = predictors)
...
swd2csv(data, file_name = "data_swd_Leucobryum_aduncum.csv")
...
```{r Response curves}
plotResponse(tuned_model, var = "wc2_6", type = "cloglog", only_presence = TRUE, marginal = TRUE, fun = mean, rug = TRUE)
ggsave("response_curve_wc2_6.png", width = 7, height = 7, units = "cm")
ggsave("response_curve_wc2_6.pdf", width = 7, height = 7, units = "cm")
